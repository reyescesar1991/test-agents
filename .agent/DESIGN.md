...
- **Charcoal Ink** (#18181B) — Primary text. Deep dark-gray depth — never pure black
- **Steel Secondary** (#71717A) — Body text, descriptions, metadata. Mid-gray warmth
...
- Pure Black (#000000) — always Off-Black or deep dark-gray only
- Oversaturated accents above 80% saturation
- Mixed warm/cool gray systems within one project

## 3. Typography Rules
- **Display:** `Geist`, `Satoshi`, `Cabinet Grotesk`, or `Outfit` — Track-tight (`letter-spacing: -0.025em`), controlled fluid scale, weight-driven hierarchy (700–900). Not screaming. Leading compressed (`line-height: 1.1`). Alternatives forced — `Inter` is BANNED for premium contexts
- **Body:** Same family at weight 400 — Relaxed leading (`line-height: 1.65`), `max-width: 65ch`, Steel Secondary color (#71717A)
...
* **Buttons:** Flat surface, no outer glow. Primary: accent fill with white text. Secondary: ghost/outline. Active state: `transform: translateY(-1px);` or `transform: scale(0.98);` for tactile push. Hover: subtle background shift, never glow
* **Cards/Containers:** Generously rounded corners (`border-radius: 2.5rem`). Pure white fill. Whisper border (`1px solid`, semi-transparent). Diffused shadow (`box-shadow: 0 20px 40px -15px rgba(0,0,0,0.05)`). Internal padding `2rem`–`2.5rem`. Used ONLY when elevation communicates hierarchy — high-density layouts replace cards with `border-top` dividers or negative space
* **Inputs/Forms:** Label positioned above input. Helper text optional. Error text below in Deep Rose. Focus ring in accent color, `outline-offset: 2px`. No floating labels. Standard `gap: 0.5rem` between label-input-error stack
* **Navigation:** Sleek, `position: sticky`. Icons scale on hover (Dock Magnification optional). No hamburger on desktop. Clean horizontal with generous spacing
...
- **No Overlapping Elements:** Text must never overlap images or other text. Every element has its own clear spatial zone. No `z-index` stacking of content layers, no `position: absolute` headlines over images. Clean separation always
...
- **No Overlapping:** Elements must never overlap each other. No `position: absolute` layers stacking content on content. Every element occupies its own grid cell or flow position. Clean, separated spatial zones
...
- **Containment:** All content within `max-width: 1400px`, centered. Generous horizontal padding (`padding: 1rem` mobile, `2rem` tablet, `4rem` desktop)
...
- **Cards & Grids:** Bento grids and asymmetric layouts revert to stacked single-column cards with full-width. Maintain internal padding (`padding: 1rem`)
...
- No pure black (`#000000`) — Off-Black or deep dark-gray only
...
- No generic `shadcn/ui` defaults (or any pre-made library defaults) — customize `border-radius`, colors, `box-shadow` to match this system
